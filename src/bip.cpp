...
int bip_send_pdu(
    ...
    if ((dest->net == BACNET_BROADCAST_NETWORK) || (dest->mac_len == 0)) {
        ...
        if (bvlc_get_last_registration_status() == SUCCESS) {
            mtu[1] = BVLC_DISTRIBUTE_BROADCAST_TO_NETWORK;
            address.s_addr = bvlc_get_remote_bbmd_address();
            port = bvlc_get_remote_bbmd_sin_port();
//            trigger(FD_REGISTRATION_SUCCESS);
#if PRINT_ENABLED
            printf("BIP: Sent Distribute-Broadcast-to-Network.\n");
#endif
        } else {
            mtu[1] = BVLC_ORIGINAL_BROADCAST_NPDU;
            address.s_addr = BIP_Broadcast_Address.s_addr;
            port = BIP_Port;
//            trigger(FD_REGISTRATION_FAIL);
#if PRINT_ENABLED
            printf("BIP: Sent Original-Broadcast-NPDU.\n");
#endif
        }
