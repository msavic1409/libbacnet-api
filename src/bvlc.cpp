...
// Last BBMD result code
enum LastRegistrationStatusCode registrationStatus = NO_REGISTRATION;

bool amIReceivingBVLCMessages = false;
 ...
 */
static void bvlc_internet_to_bacnet_address(BACNET_ADDRESS* src, struct sockaddr_in* sin) {
    if (src && sin) {
 ...
 */
static int bvlc_encode_bip_address(uint8_t* pdu, struct in_addr* address, uint16_t port) {
    int len = 0;
 ...
 * @return number of bytes decoded
 */
static int bvlc_decode_bip_address(uint8_t* pdu, struct in_addr* address, uint16_t* port) {
    int len = 0;
 ...
 * @return number of bytes encoded
 */
static int bvlc_encode_address_entry(uint8_t* pdu, struct in_addr* address, uint16_t port, struct in_addr* mask) {
    int len = 0;
...
static int bvlc_encode_read_fdt_ack(uint8_t* pdu, uint16_t max_pdu) {
    ...
    for (i = 0; i < MAX_FD_ENTRIES; i++) {
        if (FD_Table[i].valid) {
            ...
            len = bvlc_encode_bip_address(&pdu[pdu_len], &FD_Table[i].dest_address, FD_Table[i].dest_port);
            pdu_len += len;
 ...
 * @return number of bytes encoded
 */
int bvlc_encode_original_unicast_npdu(uint8_t* pdu, uint8_t* npdu, unsigned npdu_length) {
 ...
 * @return number of bytes encoded
 */
int bvlc_encode_original_broadcast_npdu(uint8_t* pdu, uint8_t* npdu, unsigned npdu_length) {
    int len = 0;    /* return value */
    unsigned i = 0; /* loop counter */
    uint16_t BVLC_length = 0;
 ...
 */
static bool bvlc_register_foreign_device(struct sockaddr_in* sin, uint16_t time_to_live) {
    unsigned i = 0;
    bool status = false;
    ...
    for (i = 0; i < MAX_FD_ENTRIES; i++) {
        if (FD_Table[i].valid) {
            if ((FD_Table[i].dest_address.s_addr == sin->sin_addr.s_addr) && (FD_Table[i].dest_port == sin->sin_port)) {
                status = true;
...
int bvlc_send_mpdu(struct sockaddr_in* dest, uint8_t* mtu, uint16_t mtu_len) {
    ...
    /* Send the packet */
    return sendto(bip_socket(), (char*)mtu, mtu_len, 0, (struct sockaddr*)&bvlc_dest, sizeof(struct sockaddr));
}
 ...
 */
static void bvlc_bdt_forward_npdu(struct sockaddr_in* sin, uint8_t* npdu, uint16_t npdu_length, bool original) {
    uint8_t mtu[MAX_MPDU] = {0};
    uint16_t mtu_len = 0;
    unsigned i = 0; /* loop counter */
    ...
    if (BVLC_NAT_Handling && original) {
        ...
        mtu_len = (uint16_t)bvlc_encode_forwarded_npdu(&mtu[0], (uint16_t)sizeof(mtu), &nat_addr, npdu, npdu_length);
    } else {
        mtu_len = (uint16_t)bvlc_encode_forwarded_npdu(&mtu[0], (uint16_t)sizeof(mtu), sin, npdu, npdu_length);
    }
    ...
    for (i = 0; i < MAX_BBMD_ENTRIES; i++) {
        if (BBMD_Table[i].valid) {
            ...
            bip_dest.sin_addr.s_addr = ((~BBMD_Table[i].broadcast_mask.s_addr) | BBMD_Table[i].dest_address.s_addr);
            bip_dest.sin_port = BBMD_Table[i].dest_port;
            /* don't send to my broadcast address and same port */
            if ((bip_dest.sin_addr.s_addr == bip_get_broadcast_addr()) && (bip_dest.sin_port == bip_get_port())) {
                continue;
            }
            /* don't send to my ip address and same port */
            if ((bip_dest.sin_addr.s_addr == bip_get_addr()) && (bip_dest.sin_port == bip_get_port())) {
                continue;
            }
             ...
             * a loop.
             */
            if (BVLC_NAT_Handling && (bip_dest.sin_addr.s_addr == BVLC_Global_Address.s_addr) &&
 ...
 * @param original - was the message an original (not forwarded)
 */
static void bvlc_fdt_forward_npdu(struct sockaddr_in* sin, uint8_t* npdu, uint16_t npdu_length, bool original) {
    uint8_t mtu[MAX_MPDU] = {0};
    uint16_t mtu_len = 0;
    unsigned i = 0; /* loop counter */
    struct sockaddr_in bip_dest = {0};
    ...
    for (i = 0; i < MAX_FD_ENTRIES; i++) {
        if (FD_Table[i].valid && FD_Table[i].seconds_remaining) {
            ...
            if ((bip_dest.sin_addr.s_addr == sin->sin_addr.s_addr) && (bip_dest.sin_port == sin->sin_port)) {
                continue;
            }
            /* NAT router port forwards BACnet packets from global IP to us.
             * Packets sent to that global IP by us would end up back, creating
 ...
 */
static int bvlc_send_result(struct sockaddr_in* dest, /* the destination address */
...
static bool bvlc_bdt_member_mask_is_unicast(struct sockaddr_in* sin) {
    ...
    for (i = 0; i < MAX_BBMD_ENTRIES; i++) {
        if (BBMD_Table[i].valid) {
            ...
            /* Skip ourself*/
            if ((BBMD_Table[i].dest_address.s_addr == bip_get_addr()) && (BBMD_Table[i].dest_port == bip_get_port())) {
                continue;
            }
 ...
 */
uint16_t bvlc_receive(BACNET_ADDRESS* src, uint8_t* npdu, uint16_t max_npdu, unsigned timeout) {
    ...
    if (select(max + 1, &read_fds, NULL, NULL, &select_timeout) > 0) {
        received_bytes = recvfrom(bip_socket(), (char*)&npdu[0], max_npdu, 0, (struct sockaddr*)&sin, &sin_len);
    } else {
        return 0;
    }
    ...
    case BVLC_READ_BROADCAST_DIST_TABLE:
        ...
        if (bvlc_send_bdt(&sin) <= 0) {
            bvlc_send_result(&sin, BVLC_RESULT_READ_BROADCAST_DISTRIBUTION_TABLE_NAK);
        }
        /* not an NPDU */
        npdu_len = 0;
        break;
    ...
    case BVLC_FORWARDED_NPDU:
        ...
        bvlc_decode_bip_address(&npdu[4], &original_sin.sin_addr, &original_sin.sin_port);
    ...
    case BVLC_READ_FOREIGN_DEVICE_TABLE:
        ...
        if (bvlc_send_fdt(&sin) <= 0) {
            bvlc_send_result(&sin, BVLC_RESULT_READ_FOREIGN_DEVICE_TABLE_NAK);
        }
        /* not an NPDU */
        npdu_len = 0;
        break;
    ...
    case BVLC_ORIGINAL_UNICAST_NPDU:
        ...
        if ((sin.sin_addr.s_addr == bip_get_addr()) && (sin.sin_port == bip_get_port())) {
            ...
            npdu_len = 0;
        } else if (BVLC_NAT_Handling && (sin.sin_addr.s_addr == BVLC_Global_Address.s_addr) &&
 ...
 */
int bvlc_send_pdu(BACNET_ADDRESS* dest, BACNET_NPDU_DATA* npdu_data, uint8_t* pdu, unsigned pdu_len) {
    struct sockaddr_in bvlc_dest = {0};
    uint8_t mtu[MAX_MPDU] = {0};
    uint16_t mtu_len = 0;
 ...
 */
static int bvlc_encode_register_foreign_device(uint8_t* pdu, uint16_t time_to_live_seconds) {
    int len = 0;
 ...
 */
int bvlc_register_with_bbmd(uint32_t bbmd_address, uint16_t bbmd_port, uint16_t time_to_live_seconds) {
    uint8_t mtu[MAX_MPDU] = {0};
    uint16_t mtu_len = 0;
    int retval = 0;

    // Allow receiving of 0x0000 from BBMD
    amIReceivingBVLCMessages = true;
    ...
    mtu_len = (uint16_t)bvlc_encode_register_foreign_device(&mtu[0], time_to_live_seconds);
 ...
 */
int bvlc_for_non_bbmd(struct sockaddr_in* sout, uint8_t* npdu, uint16_t received_bytes) {
    uint16_t result_code = 0; /* aka, BVLC_RESULT_SUCCESSFUL_COMPLETION */

    BVLC_Function_Code = static_cast<BACNET_BVLC_FUNCTION>(npdu[1]); /* The BVLC function */
    switch (BVLC_Function_Code) {
    case BVLC_RESULT:
        if (received_bytes >= 6) {
            ...
            if (BVLC_Result_Code == BVLC_RESULT_SUCCESSFUL_COMPLETION && amIReceivingBVLCMessages) {
#if PRINT_ENABLED
                printf("I am a foreign device.\n");
#endif
                bacnet::notifier::trigger_event(bacnet::notifier::FD_REGISTRATION_SUCCESS);
                registrationStatus = SUCCESS;
                amIReceivingBVLCMessages = false;
            } else if (BVLC_Result_Code == BVLC_RESULT_REGISTER_FOREIGN_DEVICE_NAK) {
#if PRINT_ENABLED
                printf("I am not a foreign device.\n");
#endif
                registrationStatus = FAIL;
                bacnet::notifier::trigger_event(bacnet::notifier::FD_REGISTRATION_FAIL);
    ...
    if (result_code > 0) {
        bvlc_send_result(sout, static_cast<BACNET_BVLC_RESULT>(result_code));
...
in_port_t bvlc_get_remote_bbmd_sin_port() {
    return Remote_BBMD.sin_port;
}

in_addr_t bvlc_get_remote_bbmd_address() {
    return Remote_BBMD.sin_addr.s_addr;
}

void bvlc_reset_remote_bbmd_settings() {
    Remote_BBMD.sin_addr.s_addr = 0;
    Remote_BBMD.sin_port = 0;
}
...
/* copy the source internet address to the BACnet address */
/* FIXME: IPv6? */
static void bvlc_bacnet_to_internet_address(struct sockaddr_in* sin, /* source address in network order */
